defmodule Tymeslot.Security.FieldValidators.IntegrationNameValidatorTest do
  ...
  describe "validate/2" do
    ...
    test "returns error for short names" do
      assert {:error, "Integration name must be at least 2 characters"} = IntegrationNameValidator.validate("A")
      # trimmed check
      assert {:error, "Integration name must be at least 2 characters"} = IntegrationNameValidator.validate(" A ")
    end
    ...
    test "returns error for long names" do
      ...
      assert {:error, "Integration name must be 100 characters or less"} = IntegrationNameValidator.validate(long_name)
    end
    ...
    test "returns error for non-binary values" do
      ...
      assert {:error, "Integration name must be text"} = IntegrationNameValidator.validate(%{key: "value"})
    end
  end
end
