defmodule Tymeslot.Security.FieldValidators.UsernameValidatorTest do
  ...
  describe "validate/2" do
    ...
    test "rejects nil or empty username" do
      ...
      assert {:error, "Username must be at least 3 characters long"} = UsernameValidator.validate("   ")
    end
    ...
    test "rejects usernames that are too short" do
      assert {:error, "Username must be at least 3 characters long"} = UsernameValidator.validate("ab")
      assert {:error, "Username must be at least 3 characters long"} = UsernameValidator.validate("a")
    end

    test "rejects usernames that are too long" do
      long_username = String.duplicate("a", 31)
      assert {:error, "Username must be at most 30 characters long"} = UsernameValidator.validate(long_username)
    end

    test "rejects invalid characters" do
      error_msg = "Username must start with a letter or number and contain only lowercase letters, numbers, underscores, and hyphens"
      assert {:error, ^error_msg} = UsernameValidator.validate("JohnDoe") # uppercase
      assert {:error, ^error_msg} = UsernameValidator.validate("john doe") # space
      assert {:error, ^error_msg} = UsernameValidator.validate("john@doe") # special char
      assert {:error, ^error_msg} = UsernameValidator.validate("_johndoe") # starts with underscore
      assert {:error, ^error_msg} = UsernameValidator.validate("-johndoe") # starts with hyphen
    end

    test "rejects reserved words" do
      reserved_words = ["admin", "api", "login", "root", "test"]
      for word <- reserved_words do
        assert {:error, "This username is reserved and cannot be used"} = UsernameValidator.validate(word)
      end
    end

    test "allows custom min/max length via options" do
      assert :ok = UsernameValidator.validate("ab", min_length: 2)
      assert {:error, "Username must be at most 5 characters long"} = UsernameValidator.validate("toolong", max_length: 5)
    end
  end
end
