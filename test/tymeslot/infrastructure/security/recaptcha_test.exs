defmodule Tymeslot.Infrastructure.Security.RecaptchaTest do
  ...
  describe "verify/2" do
    test "returns :ok with score when Google returns success" do
      ...
      assert {:ok, %{score: 0.9, action: "login", hostname: "localhost"}} = Recaptcha.verify(token)
    end
    ...
    test "returns :error when action mismatches" do
      ...
      assert {:error, :recaptcha_action_mismatch} = Recaptcha.verify(token, expected_action: "login")
    end
    ...
    test "returns :error when hostname mismatches" do
      ...
      assert {:error, :recaptcha_hostname_mismatch} = Recaptcha.verify(token, expected_hostnames: ["correct.com"])
    end
  ...
  describe "maybe_put_remote_ip/2" do
    test "adds remoteip when valid IPv4" do
      params = %{"foo" => "bar"}
      assert %{"foo" => "bar", "remoteip" => "1.2.3.4"} = Recaptcha.maybe_put_remote_ip(params, "1.2.3.4")
    end

    test "adds remoteip when valid IPv6" do
      params = %{"foo" => "bar"}
      assert %{"foo" => "bar", "remoteip" => "2001:0db8:85a3:0000:0000:8a2e:0370:7334"} = Recaptcha.maybe_put_remote_ip(params, "2001:0db8:85a3:0000:0000:8a2e:0370:7334")
    end
    ...
    test "does not add remoteip when invalid or blank" do
      ...
      assert ^params = Recaptcha.maybe_put_remote_ip(params, "fe80::1%eth0") # Scope ID rejected
    end
  ...
  describe "validation helpers" do
    test "validate_min_score/2" do
      ...
      assert {:error, :recaptcha_invalid_score} = Recaptcha.validate_min_score("not a number", 0.4)
      assert {:error, :recaptcha_configuration_error} = Recaptcha.validate_min_score(0.5, "not a number")
    end
  end
end
