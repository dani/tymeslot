defmodule Tymeslot.Integrations.Common.ErrorHandlerTest do
  ...
  describe "handle_with_logging/2" do
    ...
    test "logs and returns error for 2-tuple failure" do
      log = capture_log(fn ->
        assert {:error, "fail"} = ErrorHandler.handle_with_logging(fn -> {:error, "fail"} end, operation: "op", provider: "prov")
      end)
    ...
    test "logs and returns normalized error for 3-tuple failure" do
      log = capture_log(fn ->
        assert {:error, "timeout"} = ErrorHandler.handle_with_logging(fn -> {:error, :net, "timeout"} end)
      end)
    ...
    test "suppresses logging for specified errors" do
      log = capture_log(fn ->
        assert {:error, :ignored} = ErrorHandler.handle_with_logging(fn -> {:error, :ignored} end, suppress_errors: [:ignored])
      end)
      assert log == ""
    end
  ...
  describe "handle_integration_error/3" do
    test "translates and logs error" do
      log = capture_log(fn ->
        assert {:error, :timeout, translated} = ErrorHandler.handle_integration_error(:timeout, "google")
  ...
  describe "with_error_handling/3" do
    ...
    test "handles error by translating it" do
      capture_log(fn ->
        assert {:error, :timeout, translated} = ErrorHandler.with_error_handling("google", "op", fn -> {:error, :timeout} end)
        assert translated.category == :network
      end)
    end
    ...
    test "handles exceptions by translating them" do
      capture_log(fn ->
        assert {:error, "boom", translated} = ErrorHandler.with_error_handling("google", "op", fn -> raise "boom" end)
  ...
  describe "handle_database_error/1" do
    test "handles changeset errors" do
      changeset = Ecto.Changeset.change({%{}, %{name: :string}}, %{name: "test"})
      changeset = Ecto.Changeset.add_error(changeset, :name, "too short")
  ...
  describe "handle_http_error/2" do
    ...
    test "parses google-specific error body" do
      ...
      assert {:error, "HTTP 401: Google error message"} = ErrorHandler.handle_http_error({:ok, resp}, :google)
    end

    test "parses microsoft-specific error body" do
      body = %{"error" => %{"message" => "Microsoft error message"}}
      resp = %{status: 403, body: body}
      assert {:error, "HTTP 403: Microsoft error message"} = ErrorHandler.handle_http_error({:ok, resp}, :outlook)
    end
  end
  ...
  describe "error category mapping for 401 and 403" do
    test "translates 401 to authentication category" do
      capture_log(fn ->
        assert {:error, {:http_error, 401}, translated} = ErrorHandler.handle_integration_error({:http_error, 401}, "google")
    ...
    test "translates 403 to permission category" do
      capture_log(fn ->
        assert {:error, {:http_error, 403}, translated} = ErrorHandler.handle_integration_error({:http_error, 403}, "google")
