defmodule TymeslotWeb.Live.Scheduling.Handlers.TimezoneHandlerComponentTest do
  ...
  test "handle_timezone_search updates search term from different param formats" do
    ...
    {:ok, updated} = TimezoneHandlerComponent.handle_timezone_search(socket, %{"search" => "London"})
    assert updated.assigns.timezone_search == "London"
    
    {:ok, updated} = TimezoneHandlerComponent.handle_timezone_search(socket, %{"value" => "Paris"})
    assert updated.assigns.timezone_search == "Paris"
    
    {:ok, updated} = TimezoneHandlerComponent.handle_timezone_search(socket, %{"_target" => ["search"], "search" => "Berlin"})
