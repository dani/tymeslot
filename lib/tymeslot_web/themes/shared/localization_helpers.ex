defmodule TymeslotWeb.Themes.Shared.LocalizationHelpers do
  ...
  def format_time_by_locale(dt) do
    case gettext("time_format_type") do
      "12h" -> Elixir.Calendar.strftime(dt, "%-I:%M %p")
      _ -> Elixir.Calendar.strftime(dt, "%H:%M")
    end
  end
  ...
  defp format_minutes(minutes) when minutes < 60, do: gettext("%{minutes} minutes", minutes: minutes)
  ...
  defp format_minutes(minutes) do
    ...
    hour_text = if hours == 1, do: gettext("1 hour"), else: gettext("%{count} hours", count: hours)
    minute_text = if mins == 1, do: gettext("1 minute"), else: gettext("%{count} minutes", count: mins)
